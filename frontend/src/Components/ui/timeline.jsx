...
export const Timeline = ({
  ...
  return (
    <div
      className="w-full relative h-fit bg-white rounded-lg dark:bg-neutral-950 font-sans md:px-10"
      ref={containerRef}
    >
      <div className="flex p-6">
        <div className="left w-1/2 flex flex-col gap-4">
          <h2 className="text-lg md:text-4xl text-black dark:text-white max-w-4xl">{title}</h2>
          ...
          {isActivePathway ? (
            <>
              <div className="flex items-center justify-between">
                <p className="text-xl">
                  Your Progress Till Now:{' '}
                  <span className="text-blue-500">{parseInt(percentageComplete) + '%'}</span>
                </p>
                <div className="StartBox ml-auto">
                  <PausePathwayModal>
                    <Button>Pause your learning journey for now</Button>
                  </PausePathwayModal>
                </div>
              </div>
              <TooltipProvider>
                <Tooltip>
                  ...
                  <TooltipContent>
                    <p className="text-base">
                      {'Your have completed ' + parseInt(percentageComplete) + '% of this pathway.'}
                    </p>
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
            ...
            <div className="flex gap-2">
              {isPaused ? (
                <>
                  <div className="StartBox ml-auto">
                    <ResumePathwayModal>
                      <Button>Resume Your Learning Journey</Button>
                    </ResumePathwayModal>
                  </div>
                  <div className="StartBox ml-auto">
                    <RestartPathwayModal>
                      <Button>Restart Your Learning Journey</Button>
                    </RestartPathwayModal>
                  </div>
                </>
              ) : (
      ...
      <div ref={ref} className="relative max-w-7xl mx-auto pb-20">
        {data.map((item, index) => (
          <div key={index} className="flex justify-start pt-10 md:pt-60 md:gap-10">
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start lg:max-w-sm">
              <div className="h-10 absolute left-3 md:left-3 w-10 rounded-full bg-white dark:bg-black flex items-center justify-center">
                <div className="h-4 w-4 rounded-full bg-gradient-to-r from-purple-500 to-blue-500 border border-blue-700 p-2" />
              </div>
              <h3 className="hidden xl:inline-block text-xl xl:pl-20 md:text-3xl font-bold bg-gradient-to-r from-purple-500 to-blue-500 text-transparent bg-clip-text">
                {item.title}
              </h3>
            </div>
            <div className="relative pl-20 pr-4 md:pl-4 w-full">
              <h3 className="xl:hidden inline-block text-2xl mb-4 text-left font-bold bg-gradient-to-r from-purple-500 to-blue-500 text-transparent bg-clip-text">
                {item.title}
              </h3>
              {item.content}{' '}
            </div>
        ...
        >
          <div
            className="bg-gradient-to-b from-purple-500 to-blue-500 w-full"
            style={{ height: `${heightOfProgressTracer}` }}
          ></div>
        </div>
      </div>
    </div>
