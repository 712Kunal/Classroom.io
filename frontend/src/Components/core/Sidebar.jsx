      ...
      <div className="main flex flex-col gap-2 flex-grow">
        <SidebarSection heading={'Library'}>
          {routes.map((route, index) => (
            <SidebarItem key={index} open={isOpen} link={route.path} name={route.name} active={pathname === route.path} icon={route.icon} />
          ))}
        </SidebarSection>
        <SidebarSection heading={'Pathways'}>
          {pathways.map((pathway, index) => (
            <SidebarItem key={index} open={isOpen} link={`/app/library/pathways/${pathway.id}`} name={pathway.name} active={pathname === `/app/library/pathways/${pathway.id}`} />
          ))}
        </SidebarSection>
      </div>
...
const SidebarHeader = ({ open, setOpen }) => {
  return (
    <div className='header flex justify-between p-2 h-12'>
      ...
      <div className="sidebarToggle grid place-items-center hover:text-violet-500" onClick={() => setOpen(!open)}>
...
const SidebarFooter = ({ user, open }) => {
  return (
    <div className='footer w-full flex justify-between p-2 gap-2 self-end'>
      ...
      {
        open && <div className="user-details flex flex-col text-sm">
...
const SidebarItem = ({ open, link, name, active, icon }) => {
  return (
    <li className='flex items-center'>
      <Link to={link} className={`flex items-center gap-2 w-full border-2 border-neutral-700 p-2 text-xl rounded-lg ${active ? 'bg-violet-950 text-white' : 'hover:translate-x-2 hover:bg-violet-950'}`}>
