...
function BackgroundEducation({ user }) {
      ...
      <Accordion.Item value={valueKey}>
        <Accordion.Header>
          ...
          >
            {label}
            <span className="ml-2">
              {open[valueKey] ? <FaChevronUp size={20} /> : <FaChevronDown size={20} />}
            </span>
          </Accordion.Trigger>
        </Accordion.Header>
        <Accordion.Content>
  ...
  return (
    <Accordion.Root collapsible>
      {renderAccordionItem('Academic Background', 'Field_Of_Study', 'indigo', 'Field of Study')}

      {renderAccordionItem('Qualifications', 'Degree', 'green', 'Degree')}

      {renderAccordionItem('Experience Level', 'Years_Experience', 'red', 'Years of Experience')}
...
export function Achievements({ user }) {
  return (
    <div
      className="sm:p-6 sm:rounded-lg sm:shadow-md sm:shadow-gray-300 sm:rounded-xl p-0 shadow-none lg:p-8 lg:rounded-2xl lg:shadow-xl lg:shadow-gray-500 transition-all duration-300 
    ...
    >
      ...
      <div className="flex items-center space-x-4 mb-6">
        <p className="text-lg text-gray-600">
          <strong>Points:🪙</strong> {user.pointsAwarded}
        </p>
      </div>
            ...
            {user.badgesAwarded.map((badge, index) => (
              ...
              >
                <img
                  ...
                  loading="lazy"
                />
                <div className="absolute inset-0 bg-transparent transition-all duration-300 opacity-0 hover:opacity-100 hover:shadow-[0_0_20px_5px_rgba(0,0,0,0.5)] hover:shadow-indigo-500/50 dark:hover:shadow-[0_0_20px_5px_rgba(255,255,255,0.7)] dark:hover:shadow-purple-500/70"></div>
              </div>
            ))}
          </div>
        )}
...
export function SkillsAndHobbies({ user }) {
  ...
  const getColorClass = (item, type) => {
    const baseClasses =
      'h-8 px-3 w-max flex gap-2 items-center rounded-full focus:text-white focus:bg-opacity-75 active:bg-opacity-90 dark:text-gray-300 dark:active:bg-gray-600';
    const colorMap = {
      skills: {
