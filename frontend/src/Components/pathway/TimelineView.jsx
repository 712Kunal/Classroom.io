...
const TimelineView = () => {
  ...
  const { topic, description, duration, startDate, endDate, isActive, haveBeenPaused } =
    pathway.data;
  ...
  return (
    <div className="p-2 w-full h-svh" style={{ position: 'relative', top: '0px' }}>
      <Timeline
        data={data}
        title={topic}
...
const Interval = ({ data, pathway }) => {
  const {
    summary: title,
    tasks,
    pathwayStartDate: startDate,
    pathwayEndDate: endDate,
    intervalNumber
  } = data;
  ...
  const getTaskState = (scheduledDate, completedDate, isDone, lateMark) => {
    ...
    if (isDone) {
      return completedDate <= scheduledDate ? 'completedOnTime' : 'completedLate';
    }
    ...
    <Card className="bg-neutral-100 dark:bg-neutral-900 shadow-none hover:shadow-lg hover:shadow-blue-300 transition-all duration-300 ease-in-out">
      <CardHeader className="flex flex-col gap-2">
        ...
        <CardDescription className="flex gap-2">
          ...
          {isIntervalComplete() && (
            <Badge className="flex gap-2 w-fit bg-green-400 text-black hover:bg-neutral-300">
              <CircleCheck size={16} />
              <Separator orientation="vertical" />
                  ...
                  <Card className="bg-neutral-200 dark:bg-neutral-700 border-none shadow-none h-full w-full">
                    ...
                    <CardContent>
                      <div className="flex flex-col gap-2 justify-center items-start md:justify-start md:items-center md:flex-row text-sm">
                        ...
                        {task.scheduledDate && (
                          <div
                            className={`status flex gap-2 items-center ${taskStateToObject[taskState].color}`}
                          >
                ...
                <DialogHeader className="flex flex-col gap-2">
                  ...
                  <div className="flex gap-2 justify-stretch w-fit">
                    <div className="left flex flex-col gap-2">
                      ...
                      <div className="flex gap-2 text-bold mt-5">
                        <Database /> Refer these resources:
                      </div>
                      <div className="rounded-md flex gap-2">
                        {task.resources.map((resource, index) => (
                          <div
                            key={index}
                            className="flex gap-2 items-center border-2 border-neutral-500 dark:border-neutral-300 rounded-md p-2 hover:scale-105 cursor-pointer"
                          >
                            <a
                              href={resource.link}
                              target="_blank"
                              className="text-blue-400 hover:text-blue-500 flex gap-2"
                            >
                    ...
                    <div className="right flex flex-col items-start justify-start gap-2">
                      ...
                      {task.completedDate && (
                        <div className="completedDate flex gap-2 items-center">
                      ...
                      {(taskState === 'pending' || taskState === 'lateMark') && (
                        <Button
                          ...
                          onClick={() => handleMarkAsDone(task.taskNumber)}
                        >
