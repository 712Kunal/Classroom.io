            ...
            <span>
              <Button variant="outline" size="icon" onClick={toggleCollapseExpandAll}>
                {expandedTasks.length > 0 ? (
                  <CopyMinus size={24} className="text-gray-800 dark:text-gray-500" />
                ) : (
                  <CopyPlus size={24} className="text-gray-500" />
                )}
              </Button>
            </span>
          </div>
              ...
              <AccordionItem key={task.taskNumber} value={`interval-${task.taskNumber}`}>
                <AccordionTrigger onClick={() => toggleInterval(`interval-${task.taskNumber}`)}>
                  <div className="flex items-center gap-2">
                    <Badge
                      variant={expandedTasks.includes(task.taskNumber) ? 'default' : 'outline'}
                    >
                ...
                <AccordionContent>
                  <div className="pl-4 border-l-2 border-gray-200 ml-4 mt-2">
                    <Card
                      className={`mb-4 p-4 ${taskStateToBgColorMap[taskState]} border-2 ${taskStateToBorderColorMap[taskState]} ${['completedOnTime', 'completedLate'].includes(taskState) ? 'opacity-75' : ''}`}
                    >
                      <div className="top flex items-center justify-between">
                        <div className="left">
                          <h3
                            className={`text-lg font-semibold mb-2 ${taskStateToTextColorMap[taskState]}`}
                          >
                            {task.taskTitle}
                          </h3>
                        ...
                        <div className="flex flex-col items-end gap-2 justify-center">
                          ...
                          {task.completedDate && (
                            <div
                              className={`flex items-center justify-between ${taskStateToTextColorMap[taskState]} gap-2`}
                            >
                          ...
                          {task.resources.map((resource, index) => (
                            <li key={index}>
                              <a href={resource.link} target="_blank" rel="noopener noreferrer">
                                <Badge
                                  variant="outline"
                                  className={`flex gap-2 items-center ${ResourceTypeToBgColorMap[resource.type]} border-2 ${ResourceTypeToColorMap[resource.type]}`}
                                >
                      ...
                      <p className="mt-2">
                        <strong className="text-gray-700 dark:text-gray-200">Outcome:</strong>{' '}
